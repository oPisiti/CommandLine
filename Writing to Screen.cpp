...
public:
	...
	void drawCharacter(int32_t x, int32_t y, uint8_t charPos, std::vector<std::vector<uint8_t>>& data, unsigned int scale = 1,
		...
		for (int hei = 0; hei < charHeight; hei++) {
			...
			for (int wid = 0; wid < charWidth; wid++) {
				...
				if ((data[charPos][curPosY] & (1 << curPosX)) >> curPosX) {
